...
const AdminBatchesList: React.FC = () => {
  ...
  const columns = [
    {
      ...
      render: (ingredient?: IngredientModel) => (
        ingredient ? (
          <Link href={`/admin/inventory/ingredients/${ingredient.id}`}>
        ...
        <Space size="small">
          <Tooltip title="Xem chi tiết">
            <Button 
              icon={<EyeOutlined />} 
              size="small"
              onClick={() => router.push(`/admin/inventory/batches/${record.id}`)}
            />
          </Tooltip>
          <Tooltip title="Xuất kho">
            <Button 
              ...
              onClick={() => router.push(`/admin/inventory/exports/create?batch=${record.id}`)}
            />
          </Tooltip>
        </Space>
      ),
