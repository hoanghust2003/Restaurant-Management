...
const MultiBatchExportForm: React.FC<MultiBatchExportFormProps> = ({
  ...
  const fetchBatches = async (ingredientId: string) => {
    try {
      ...
      const validBatches = data.filter((batch: BatchModel) => batch.remaining_quantity > 0);
  ...
  const addIngredientToExport = async (ingredientId: string) => {
    const ingredient = ingredients.find((i: IngredientModel) => i.id === ingredientId);
    if (!ingredient) return;

    // Check if ingredient is already added
    if (exportItems.find((item: IngredientExportItem) => item.ingredient_id === ingredientId)) {
  ...
  const removeIngredientFromExport = (ingredientId: string) => {
    setExportItems(prev => prev.filter((item: IngredientExportItem) => item.ingredient_id !== ingredientId));
  };

  const updateBatchQuantity = (ingredientId: string, batchId: string, quantity: number) => {
    setExportItems(prev => 
      prev.map((item: IngredientExportItem) => {
        if (item.ingredient_id !== ingredientId) return item;

        const batch = availableBatches[ingredientId]?.find((b: BatchModel) => b.id === batchId);
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      const invalidItems = exportItems.filter((item: IngredientExportItem) => item.totalQuantity <= 0);
      ...
      // Navigate to export details in admin
      router.push(`/admin/inventory/exports/${result.id}`);
    } catch (err: any) {
      ...
      <Card>
        ...
        >
          ...
          <Card size="small" style={{ marginBottom: 16, backgroundColor: '#fafafa' }}>
            <Row gutter={16} align="middle">
              <Col flex={1}>
                ...
                >
                  {ingredients
                    .filter((ingredient: IngredientModel) => !exportItems.find((item: IngredientExportItem) => item.ingredient_id === ingredient.id))
          ...
          {exportItems.length === 0 ? (
            ...
            <Collapse 
              defaultActiveKey={exportItems.map((_: IngredientExportItem, index: number) => index.toString())}
              style={{ marginBottom: 16 }}
            >
              {exportItems.map((item: IngredientExportItem, index: number) => (
                <Panel
                  key={index}
                  header={
                  ...
                  <div>
                    ...
                    {loading.batches ? (
                      ...
                      <div className="space-y-3">
                        {(availableBatches[item.ingredient_id] || []).map((batch: BatchModel) => {
          ...
          {exportItems.length > 0 && getTotalExportQuantity() > 0 && (
            <Card size="small" style={{ backgroundColor: '#f6ffed', border: '1px solid #b7eb8f', marginBottom: 16 }}>
              <Row gutter={16}>
                ...
                <Col span={12}>
                  <Text strong>Tổng số lô xuất: {exportItems.reduce((sum: number, item: IngredientExportItem) => sum + Object.keys(item.batches).length, 0)}</Text>
                </Col>
              </Row>
            </Card>
          )}
